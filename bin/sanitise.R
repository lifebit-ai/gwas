...
#'
#' @examples
sanitise <- function(table = NULL, col_names = FALSE, pattern = "href") {
    ...
    for (col in col_names) {
        ...
        if (grepl(pattern, col_as_string, fixed = TRUE)) {
            ...
            table[[col]] <- paste0("<a href='", table[[url]], "'", "target='blank", 
                "'>", table[[col]], "</a>")
            table <- as.data.frame(table) %>% dplyr::mutate_all(stringr::str_replace_all, 
