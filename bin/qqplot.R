...
if (is.null(args$type)) {
    args$type = getOption("bitmapType")
} else {
...
png(filename = paste0(output_tag, "_", "qqplot_ci", ".png"), width = width, height = height, 
    units = units, res = res, type = type)


GWASTools::qqPlot(analysis$P, main = paste0(output_tag, ",", "lambda=", round(lambda(analysis$P), 
    2)))
dev.off()

# check whether we can just save the R objects and also give the option for
# exporting through a future airlock
