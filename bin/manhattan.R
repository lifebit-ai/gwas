...
if (is.null(args$p_value_cutoff)) {
    args$p_value_cutoff = 0.01
} else {
    args$p_value_cutoff = as.numeric(args$p_value_cutoff)
}
if (is.null(args$title)) {
    args$title = paste0(title = "Manhattan plot with rsid annotation (p-value < ", 
        as.character(args$p_value_cutoff), ", gwas id: ", args$output_tag, ")")
} else {
    args$title = as.character(args$title)
}
...
if (is.null(args$units)) {
    ...
    args$units = as.character(args$units)
}
if (is.null(args$type)) {
    args$type = "cairo"
} else {
    args$type = as.character(args$type)
}
if (is.null(args$res)) {
    args$res = 300
} else {
    args$res = as.character(args$res)
}
if (is.null(args$cex)) {
    args$cex = 0.6
} else {
    args$cex = as.character(args$cex)
}
if (is.null(args$cex_axis)) {
    args$cex_axis = 0.6
} else {
    args$cex_axis = as.character(args$cex_axis)
}
if (is.null(args$suggestive_line)) {
    args$suggestive_line = TRUE
} else {
    args$suggestive_line = as.logical(args$suggestive_line)
}
if (is.null(args$manhattan_colour_1)) {
    args$manhattan_colour_1 = "#4e4b4c"
} else {
    args$manhattan_colour_1 = as.character(args$manhattan_colour_1)
}
if (is.null(args$manhattan_colour_2)) {
    args$manhattan_colour_2 = "#4dc5ce"
} else {
    args$manhattan_colour_2 = as.character(args$manhattan_colour_2)
}
if (is.null(args$bp_col)) {
    args$bp_col = "POS"
} else {
    args$bp_col = as.character(args$bp_col)
}
if (is.null(args$snp_col)) {
    args$snp_col = "SNPID"
} else {
    args$snp_col = as.character(args$snp_col)
}
if (is.null(args$p_val_col)) {
    args$p_val_col = "p.value"
} else {
    args$p_val_col = as.character(args$p_val_col)
}
...
png(filename = paste0(output_tag, "_", "manhattan", ".png"), width = width, height = height, 
    units = units, res = res, type = type)

manhattan_plot <- qqman::manhattan(analysis, bp = bp_col, p = p_val_col, snp = snp_col, 
    main = title, col = c(manhattan_colour_1, manhattan_colour_2), highlight = analysis[[snp_col]][analysis[[p_val_col]] < 
        p_value_cutoff], annotatePval = p_value_cutoff, suggestiveline = -log10(p_value_cutoff))
